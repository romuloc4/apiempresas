...
namespace ApiEmpresas.Services.Controllers
{
    ...
    public class EmpresasController : ControllerBase
    {
        ...
        //contrustor pra injeção de dependencia
        ...
        public IActionResult Post(EmpresaPostRequest request)
        {
            try
            {
                //vericar se o CNPJ informado já está cadastrado..
                if (_unitOfWork.EmpresaRepository.ObterPorCnpj(request.Cnpj) != null)
                    //HTTP 422 -> UNPROCESSALE ENTITY
                ...
                //HTTP 201 -> SUCCESS CREATD
                return StatusCode(201, response);
            }
            catch(Exception e)
            {
                //retornando status e mensagem de erro
                //HTTP 500 -> ERRO INTERNO NO SERVIOR
        ...
        public IActionResult Put(EmpresaPutRequest request)
        {
            try
            {
                ...
                if (empresa == null)
                    ...
                    return StatusCode(422, new { message = "empresa não encontrada, verfique o ID informado." });

                //verificando se o cnpj informado está cadastrado para outra empresa
        ...
        public IActionResult GetAll()
        {
            try
            {
                var empresa = _unitOfWork.EmpresaRepository.Consultar();
                var lista = _mapper.Map<List<EmpresaResponse>>(empresa);
